...
extern int
vsnprintf(char* __restrict __s, size_t __maxlen, const char* __restrict __format, __gnuc_va_list __arg)
...
extern char* index(const char* __s, int __c) __attribute__((__nothrow__, __leaf__))
__attribute__((__pure__)) __attribute__((__nonnull__(1)));
# 96 "/usr/include/strings.h" 3 4
extern char* rindex(const char* __s, int __c) __attribute__((__nothrow__, __leaf__))
__attribute__((__pure__)) __attribute__((__nonnull__(1)));
...
extern void
__assert_fail(const char* __assertion, const char* __file, unsigned int __line, const char* __function)
__attribute__((__nothrow__, __leaf__)) __attribute__((__noreturn__));


extern void
__assert_perror_fail(int __errnum, const char* __file, unsigned int __line, const char* __function)
__attribute__((__nothrow__, __leaf__)) __attribute__((__noreturn__));
...
# 33 "unit_tests.h"
    enabled;
    struct _test_instance_* next;
} TestInstance;

typedef struct _test_section_ {
    const char* name;
    const char* desc;
    const char* fname;
    TestInstance* test_list;
    int defined;
    int ran;
    int skipped;
    int pass;
...
# 122 "unit_tests.h"
void unit_mock_entered(TestInstance* ti, const char* name);
void unit_stub_entered(TestInstance* ti, const char* name);
...
void unit_add_section(TestGroup* group, TestSection* section);

int unit_run_tests(TestGroup* group);
void add_assert_msg(TestInstance* ti, int line, const char* fmt, ...);

TestMsg* unit_test_init_msg();
void unit_test_add_msg(const char* group, int verbo, int line, const char* name, const char* fmt, ...);
# 9 "unit_tests.c" 2

# 1 "unit_test_lists.h" 1
# 11 "unit_tests.c" 2
# 1 "memory.h" 1
...
MsgList* MsgListInit();
void MsgListDestroy(MsgList* lst);
void MsgListAdd(MsgList* lst, char* data);
char* MsgListGet(MsgList* lst, int index);
char** MsgListGetRaw(MsgList* lst);
int MsgListGetLen(MsgList* lst);
MsgList* MsgListInit() {
    MsgList* lst = (MsgList*)allocMemory(sizeof(MsgList));
    lst->len = 0;
    lst->cap = 1;
    lst->list = (char**)allocMemory(sizeof(char*) * (lst->cap));
    return lst;
}
void MsgListDestroy(MsgList* lst) {
# 13 "unit_tests.c" 3 4
...
# 13 "unit_tests.c" 3 4
             ,
             "unit_tests.c", 13, __extension__ __PRETTY_FUNCTION__);
     }))
# 13 "unit_tests.c"
    ;
    freeMemory((void*)lst->list);
    freeMemory((void*)lst);
}
void MsgListAdd(MsgList* lst, char* data) {
    if(lst->len + 1 > lst->cap) {
        lst->cap <<= 1;
        lst->list = (char**)reallocMemory((lst->list), sizeof(char*) * (lst->cap));
    }
...
# 13 "unit_tests.c" 3 4
    ((void)sizeof((
# 13 "unit_tests.c"
                  lst->len >= 0 && index < lst->len && index >= 0
# 13 "unit_tests.c" 3 4
                  ) ?
                  1 :
                  0),
...
# 13 "unit_tests.c" 3 4
             ...
             __assert_fail("lst->len >= 0 && index < lst->len && index >= 0",
                           "unit_tests.c", 13, __extension__ __PRETTY_FUNCTION__);
     }))
# 13 "unit_tests.c"
    ;
...
void MockListDestroy(MockList* lst);
void MockListAdd(MockList* lst, TestMock* data);
TestMock* MockListGet(MockList* lst, int index);
TestMock** MockListGetRaw(MockList* lst);
int MockListGetLen(MockList* lst);
MockList* MockListInit() {
    MockList* lst = (MockList*)allocMemory(sizeof(MockList));
    lst->len = 0;
    lst->cap = 1;
    lst->list = (TestMock**)allocMemory(sizeof(TestMock*) * (lst->cap));
    return lst;
}
void MockListDestroy(MockList* lst) {
# 14 "unit_tests.c" 3 4
...
# 14 "unit_tests.c" 3 4
             ,
             "unit_tests.c", 14, __extension__ __PRETTY_FUNCTION__);
     }))
# 14 "unit_tests.c"
    ;
    freeMemory((void*)lst->list);
...
void MockListAdd(MockList* lst, TestMock* data) {
    if(lst->len + 1 > lst->cap) {
        lst->cap <<= 1;
        lst->list =
        (TestMock**)reallocMemory((lst->list), sizeof(TestMock*) * (lst->cap));
    }
    lst->list[lst->len++] = data;
}
TestMock* MockListGet(MockList* lst, int index) {
...
StubList* StubListInit();
void StubListDestroy(StubList* lst);
void StubListAdd(StubList* lst, TestStub* data);
TestStub* StubListGet(StubList* lst, int index);
TestStub** StubListGetRaw(StubList* lst);
int StubListGetLen(StubList* lst);
StubList* StubListInit() {
    StubList* lst = (StubList*)allocMemory(sizeof(StubList));
    lst->len = 0;
    lst->cap = 1;
    lst->list = (TestStub**)allocMemory(sizeof(TestStub*) * (lst->cap));
    return lst;
}
void StubListDestroy(StubList* lst) {
# 15 "unit_tests.c" 3 4
...
# 15 "unit_tests.c" 3 4
             ,
             "unit_tests.c", 15, __extension__ __PRETTY_FUNCTION__);
     }))
# 15 "unit_tests.c"
    ;
    freeMemory((void*)lst->list);
...
void StubListAdd(StubList* lst, TestStub* data) {
    if(lst->len + 1 > lst->cap) {
        lst->cap <<= 1;
        lst->list =
        (TestStub**)reallocMemory((lst->list), sizeof(TestStub*) * (lst->cap));
    }
    lst->list[lst->len++] = data;
}
TestStub* StubListGet(StubList* lst, int index) {
...
TestList* TestListInit();
void TestListDestroy(TestList* lst);
void TestListAdd(TestList* lst, TestInstance* data);
TestInstance* TestListGet(TestList* lst, int index);
TestInstance** TestListGetRaw(TestList* lst);
int TestListGetLen(TestList* lst);
TestList* TestListInit() {
    TestList* lst = (TestList*)allocMemory(sizeof(TestList));
    lst->len = 0;
    lst->cap = 1;
    lst->list = (TestInstance**)allocMemory(sizeof(TestInstance*) * (lst->cap));
    return lst;
}
void TestListDestroy(TestList* lst) {
# 16 "unit_tests.c" 3 4
...
# 16 "unit_tests.c" 3 4
             ,
             "unit_tests.c", 16, __extension__ __PRETTY_FUNCTION__);
     }))
# 16 "unit_tests.c"
    ;
    freeMemory((void*)lst->list);
...
void TestListAdd(TestList* lst, TestInstance* data) {
    if(lst->len + 1 > lst->cap) {
        lst->cap <<= 1;
        lst->list =
        (TestInstance**)reallocMemory((lst->list), sizeof(TestInstance*) * (lst->cap));
    }
    lst->list[lst->len++] = data;
}
TestInstance* TestListGet(TestList* lst, int index) {
...
SectionList* SectionListInit();
void SectionListDestroy(SectionList* lst);
void SectionListAdd(SectionList* lst, TestSection* data);
TestSection* SectionListGet(SectionList* lst, int index);
TestSection** SectionListGetRaw(SectionList* lst);
int SectionListGetLen(SectionList* lst);
SectionList* SectionListInit() {
    SectionList* lst = (SectionList*)allocMemory(sizeof(SectionList));
    lst->len = 0;
    lst->cap = 1;
    lst->list = (TestSection**)allocMemory(sizeof(TestSection*) * (lst->cap));
    return lst;
}
void SectionListDestroy(SectionList* lst) {
# 17 "unit_tests.c" 3 4
...
# 17 "unit_tests.c" 3 4
             ,
             "unit_tests.c", 17, __extension__ __PRETTY_FUNCTION__);
     }))
# 17 "unit_tests.c"
    ;
    freeMemory((void*)lst->list);
...
void SectionListAdd(SectionList* lst, TestSection* data) {
    if(lst->len + 1 > lst->cap) {
        lst->cap <<= 1;
        lst->list =
        (TestSection**)reallocMemory((lst->list), sizeof(TestSection*) * (lst->cap));
    }
    lst->list[lst->len++] = data;
}
TestSection* SectionListGet(SectionList* lst, int index) {
...
GroupList* GroupListInit();
void GroupListDestroy(GroupList* lst);
void GroupListAdd(GroupList* lst, TestGroup* data);
TestGroup* GroupListGet(GroupList* lst, int index);
TestGroup** GroupListGetRaw(GroupList* lst);
int GroupListGetLen(GroupList* lst);
GroupList* GroupListInit() {
    GroupList* lst = (GroupList*)allocMemory(sizeof(GroupList));
    lst->len = 0;
    lst->cap = 1;
    lst->list = (TestGroup**)allocMemory(sizeof(TestGroup*) * (lst->cap));
    return lst;
}
void GroupListDestroy(GroupList* lst) {
# 18 "unit_tests.c" 3 4
...
# 18 "unit_tests.c" 3 4
             ,
             "unit_tests.c", 18, __extension__ __PRETTY_FUNCTION__);
     }))
# 18 "unit_tests.c"
    ;
    freeMemory((void*)lst->list);
...
void GroupListAdd(GroupList* lst, TestGroup* data) {
    if(lst->len + 1 > lst->cap) {
        lst->cap <<= 1;
        lst->list =
        (TestGroup**)reallocMemory((lst->list), sizeof(TestGroup*) * (lst->cap));
    }
    lst->list[lst->len++] = data;
}
TestGroup* GroupListGet(GroupList* lst, int index) {
...
static TestMock* unit_find_mock(TestInstance* ti, const char* name) {

    TestMock* crnt;
    for(crnt = ti->mock_list; crnt !=
# 23 "unit_tests.c" 3 4
        ((void*)0)
# 23 "unit_tests.c"
        ;
...
static TestStub* unit_find_stub(TestInstance* ti, const char* name) {

    TestStub* crnt;
    for(crnt = ti->stub_list; crnt !=
# 33 "unit_tests.c" 3 4
        ((void*)0)
# 33 "unit_tests.c"
        ;
...
void unit_mock_entered(TestInstance* ti, const char* name) {
    ...
    TestMock* mock = unit_find_mock(ti, name);
    if(mock !=
# 43 "unit_tests.c" 3 4
       ((void*)0)
# 43 "unit_tests.c"
        ...
        unit_error(ti, 0, "mock name \"%s\" does not exist", name);
}

void unit_stub_entered(TestInstance* ti, const char* name) {

    TestStub* stub = unit_find_stub(ti, name);
    if(stub !=
# 52 "unit_tests.c" 3 4
       ((void*)0)
# 52 "unit_tests.c"
        ...
        unit_error(ti, 0, "stub name \"%s\" does not exist", name);
}

void unit_add_mock(TestInstance* ti, const char* name) {

    TestMock* mock = unit_test_alloc(sizeof(TestMock));
    mock->name = unit_test_strcpy(name);
    mock->entered = 0;
...
void unit_add_stub(TestInstance* ti, const char* name) {
    ...
    TestStub* stub = unit_test_alloc(sizeof(TestStub));
    stub->name = unit_test_strcpy(name);
    stub->entered = 0;
...
void unit_add_test(TestSection* section, const char* name, TestFunc func) {
    ...
    TestInstance* test = unit_test_alloc(sizeof(TestInstance));
    test->name = unit_test_strcpy(name);
    test->func = func;
    test->enabled =
# 93 "unit_tests.c" 3 4
    1
# 93 "unit_tests.c"
    ...
    if(section->test_list ==
# 96 "unit_tests.c" 3 4
       ((void*)0)
# 96 "unit_tests.c"
    )
        section->test_list = test;
    else {
        TestInstance* tmp = section->test_list;
        do {
            tmp = tmp->next;
        } while(tmp->next !=
# 100 "unit_tests.c" 3 4
                ((void*)0)
# 100 "unit_tests.c"
        );
        tmp->next = test;
...
void unit_add_section(TestGroup* group, TestSection* section) {
    ...
    if(group->tests ==
# 108 "unit_tests.c" 3 4
       ((void*)0)
# 108 "unit_tests.c"
    )
        group->tests = section;
    else {
        TestSection* tmp = group->tests;
        do {
            tmp = tmp->next;
        } while(tmp->next !=
# 112 "unit_tests.c" 3 4
                ((void*)0)
# 112 "unit_tests.c"
        );
        tmp->next = section;
...
int unit_run_tests(TestGroup* group) {
    ...
    if(group !=
# 121 "unit_tests.c" 3 4
       ((void*)0)
# 121 "unit_tests.c"
    ) {
        if(group->tests !=
# 122 "unit_tests.c" 3 4
           ((void*)0)
# 122 "unit_tests.c"
        ) {
            TestSection* sec = group->tests;
            if(sec !=
# 124 "unit_tests.c" 3 4
               ((void*)0)
# 124 "unit_tests.c"
               && sec->enabled) {
                do {
                    TestInstance* tst = sec->test_list;
                    if(tst !=
# 127 "unit_tests.c" 3 4
                       ((void*)0)
# 127 "unit_tests.c"
                    ) {
                        do {
...
# 140 "unit_tests.c"
            ...
            fprintf(
# 144 "unit_tests.c" 3 4
            stderr
# 144 "unit_tests.c"
            ,
            "there are no tests to to be run!\n");
        }
    }
    else {
        fprintf(
# 148 "unit_tests.c" 3 4
        stderr
# 148 "unit_tests.c"
        ,
...
TestMsg* unit_test_init_msg() {
    ...
    TestMsg* msgs = unit_test_alloc(sizeof(TestMsg));
    msgs->cap = 1;
    msgs->len = 0;
    msgs->buffer = (char**)unit_test_alloc(sizeof(char*));

    return msgs;
}

static TestMsg* unit_test_msgs;
void unit_test_add_msg(const char* group, int verbo, int line, const char* name, const char* fmt, ...) {

    if(unit_test_msgs->len + 1 > unit_test_msgs->cap) {
        unit_test_msgs->cap <<= 1;
        char** tmp = (char**)unit_test_alloc(sizeof(char*) * unit_test_msgs->cap);
        memcpy(tmp, unit_test_msgs->buffer, sizeof(char*) * unit_test_msgs->cap);
        unit_test_free(unit_test_msgs->buffer);
        unit_test_msgs->buffer = tmp;
    }

    const char* format = "%s: %s:\t%d:\t";
    int len1 = snprintf(
# 176 "unit_tests.c" 3 4
    ((void*)0)
# 176 "unit_tests.c"
    ,
    0, format, group, name, line);

    va_list args;


# 180 "unit_tests.c" 3 4
    __builtin_va_start(
# 180 "unit_tests.c"
    args
# 180 "unit_tests.c" 3 4
...
# 180 "unit_tests.c"
    ;
    int len2 = vsnprintf(
# 181 "unit_tests.c" 3 4
    ((void*)0)
# 181 "unit_tests.c"
    ,
    0, fmt, args);

# 182 "unit_tests.c" 3 4
    __builtin_va_end(
# 182 "unit_tests.c"
    args
# 182 "unit_tests.c" 3 4
...
# 182 "unit_tests.c"
    ...
    char* str = unit_test_alloc(len1 + len2 + 2);
    snprintf(str, len1 + 1, format, group, name, line);


# 187 "unit_tests.c" 3 4
    __builtin_va_start(
# 187 "unit_tests.c"
    args
# 187 "unit_tests.c" 3 4
...
# 189 "unit_tests.c"
    ...
    if(verbo <= 7)
        fprintf(
# 194 "unit_tests.c" 3 4
        stderr
# 194 "unit_tests.c"
        ,
        "%s", str);

    unit_test_msgs->buffer[unit_test_msgs->len++] = str;
}
